...
// Inicializar Prisma (con manejo de conexiones optimizado para serverless)
// const prisma = new PrismaClient() // Removed because prisma is imported from lib/prisma.js

// Función auxiliar para ejecutar consultas con un cliente aislado
// async function executeWithIsolatedClient(callback) { // Removed because executeWithIsolatedClient is imported from lib/prisma.js
//   try {
...
app.get("/", async (req, res) => {
  try {
    ...
    res.json({ message: "Conexión con la base de datos establecida correctamente." })
  } catch (error) {
...
app.get("/api/rentas/completadas/:id", async (req, res) => {
  const userId = Number.parseInt(req.params.id, 10) // ID del usuario desde los parámetros de la ruta
  if (isNaN(userId)) {
...
app.get("/api/rentas/calificaciones/:id", async (req, res) => {
  const userId = Number.parseInt(req.params.id, 10) // ID del usuario obtenido desde la ruta
  if (isNaN(userId)) {
    return res.status(400).json({ error: "El ID del usuario debe ser un número válido." })
  }
  try {
...
app.delete("/api/rentas/calificaciones/:id/:idCal", async (req, res) => {
  const userId = Number.parseInt(req.params.id, 10) // ID of the calificador
  const calificacionId = Number.parseInt(req.params.idCal, 10) // ID of the calificación
...
// En su lugar, exporta la instancia de Express
export default app
